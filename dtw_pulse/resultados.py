    ...
    for res in resultados:
        ...
        if ready:
            ...
            print(
                ...
                f"Pulse cap={MAX_PULSE_ITERATIONS} iters"
            )
...
def _get_pulse_entries(hist_mode):
    """Iteraciones donde arranca un pulso de exploit (explore -> exploit)."""
    entries = []
    for i in range(len(hist_mode)):
        prev = hist_mode[i - 1] if i > 0 else "explore"
        if hist_mode[i] == "exploit" and prev != "exploit":
            entries.append(i)
    return entries
...
def generate_plots(resultados_por_mh: dict, inst: dict, save_dir: str):
    ...
    Genera 2 paneles estilo paper:
      1. Fitness superpuesto con segmentos punteados en explore + marcadores de pulso.
      2. D2 vs theta_c con los umbrales de entrada/salida del pulso.
    """
    ...
    for nombre in nombres:
        ...
        pulse_entries = _get_pulse_entries(hist_mode)
        ...
        ax1.plot(
            ...
            label=f"{nombre} (max={res['mejor_fitness']:.1f}, pulses={len(pulse_entries)})",
        )

        # Marcadores de inicio de pulso
        if pulse_entries:
            pulse_fits = [hist_fit[i] for i in pulse_entries]
            ax1.scatter(
                pulse_entries, pulse_fits, color=COLORS["pulse_marker"],
    ...
    ax1.set_title("DTW-Pulse — All MHs")
    ax1.legend(loc="lower right", framealpha=0.9)
    ax1.grid(True, alpha=0.2, linestyle=":")

    # =========================================================================
    # Panel 2: D2 vs theta_c (señal del controlador Pulse)
    # =========================================================================
    for nombre in nombres:
        res = mejores[nombre]
        ...
        if ready_iters:
            ...
            # Umbral de salida del pulso (D2 <= theta_c)
            ax2.plot(
                ready_iters, theta_c_vals, color=color,
                linewidth=0.8, linestyle=":", alpha=0.5,
                label=rf"$\theta_c$ {nombre}", zorder=2,
            )
            # Umbral de entrada por alta actividad (2*theta_c)
            high_activity = [2.0 * t for t in theta_c_vals]
            ax2.plot(
                ready_iters, high_activity, color=color,
    ...
    ax2.set_title(
        rf"DTW-Pulse — enter exploit on improvement or $D_2 > 2\theta_c$; "
        rf"exit at $D_2 \leq \theta_c$ w/o fresh improve or cap ({MAX_PULSE_ITERATIONS})"
    )
    ...
    for ext in ("png", "pdf"):
        path = f"{save_dir}/dtw_pulse_{inst_name}.{ext}"
        fig.savefig(path)
        print(f"  Saved: {path}")
...
def main():
    ...
    print("=" * 70)
    print("  RESULTADOS MAESTRO — DTW-Pulse")
    print("=" * 70)
    ...
    print(f"  Pulse cap: {MAX_PULSE_ITERATIONS} iteraciones")

    # Crear carpeta de salida con timestamp
    run_id = datetime.now().strftime("%Y%m%d_%H%M%S")
    inst_name = Path(RUTA_INSTANCIA).stem
    save_dir = f"results/dtw_pulse/todos/{inst_name}_{INDICE_INSTANCIA}/comparacion_mhs_{run_id}"
    Path(save_dir).mkdir(parents=True, exist_ok=True)
