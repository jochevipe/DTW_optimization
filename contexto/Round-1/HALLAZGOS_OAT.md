...
**Decisión de trabajo:** no repetir por ahora estas campañas de sensibilidad. Están completas en su alcance; usar sus resultados como evidencia **descriptiva y acotada**, no como prueba de robustez estadística/general. Continuar con experimentos de más instancias y ablación sin DDTW por separado. Este documento resume los resultados para que el siguiente trabajo en `dtw_discreto` no dependa de copiar los JSON de `results/`.

## Alcance y trazabilidad

La observación literal R1.4 del Revisor 1 solicita examinar el efecto de variar la ventana W=200, la banda Sakoe–Chiba, los percentiles 40/60, τ_pat=3 y π_max=5 (carta `Reviewer 1.md` en `dtw_discreto`). El estudio cambia **un factor a la vez**: seis parámetros, tres valores alternativos por parámetro y una configuración base, para 19 configuraciones por estrategia. `min_slope=2.0` permanece fijo. El diseño está descrito en `SENSIBILIDAD_OAT.md` de la rama OAT.
