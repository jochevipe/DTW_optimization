"""
Análisis individual de una MH + Binary-Hysteresis (estrategia A9).
Ejecuta 1 corrida, muestra métricas DTW por iteración, genera plot para paper.

Uso (desde la raíz del proyecto):
    python -m binary_hysteresis.run
"""
...
def print_iteration_table(res: dict, inst: dict) -> None:
    ...
    hdr = (
        f"{'it':>4} | {'fitness':>10} | {'state':>7} | {'transition':>10} | "
        f"{'D1':>7} | {'D2':>7} | {'delta':>8} | "
    ...
    for i in range(len(hist_fit)):
        ...
        if dtw.get("ready"):
            if is_entry_trigger(dtw, hist_mode[i]):
                transition = "ENTER"
            elif is_exit_trigger(dtw, hist_mode[i]):
                transition = "EXIT"
            else:
                transition = "---"
            row = (
                f"{i:4d} | {fitness:10.1f} | {mode:>7} | {transition:>10} | "
            ...
            row = (
                f"{i:4d} | {fitness:10.1f} | {mode:>7} | {'---':>10} | "
                f"{'---':>7} | {'---':>7} | {'---':>8} | "
                f"{'---':>7} | {'---':>7} | {'---':>7} | "
                f"{dtw['no_improve_len']:5d} |  ---"
            )
...
def plot_paper(res: dict, inst: dict, save_dir: str = "results/binary_hysteresis"):
    """
    ...
    if entry_triggers:
        ...
        ax1.scatter(
            ...
            label=f"Entry trigger ({len(entry_triggers)})",
            edgecolors="white", linewidths=0.5,
        )
    ...
    ax1.set_title(
        f"{mh_name} + DTW Binary-Hysteresis — Convergence (seed={res.get('semilla', '?')})"
    )
