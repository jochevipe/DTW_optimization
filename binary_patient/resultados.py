"""
Binary-Patient — decisión D2 sostenida con paciencia.
Corre todas las MHs, muestra métricas DTW en consola, y genera gráficos.

Uso (desde la raíz del proyecto):
    python -m binary_patient.resultados
"""
...
def generate_plots(resultados_por_mh: dict, inst: dict, save_dir: str):
    ...
    ax1.set_title(f"Binary-Patient — All MHs (seed={SEMILLA})")
    ax1.legend(loc="lower right", framealpha=0.9)
    ax1.grid(True, alpha=0.2, linestyle=":")

    # =========================================================================
    # Panel 2: D2 vs theta_c (señal de decisión Binary-Patient)
    # =========================================================================
    ...
    ax2.set_title(
        r"Binary-Patient — Fire after sustained $D_2 \leq \theta_c$"
    )
    ...
    # save_dir format: results/binary_patient/todos/{inst_name}_{idx}/comparacion_mhs_{run_id}
    ...
    for ext in ("png", "pdf"):
        path = f"{save_dir}/binary_patient_{inst_name}.{ext}"
        fig.savefig(path)
        print(f"  Saved: {path}")
...
def main():
    ...
    print("=" * 70)
    print("  RESULTADOS MAESTRO — Binary-Patient")
    print("=" * 70)
    ...
    save_dir = f"results/binary_patient/todos/{inst_name}_{INDICE_INSTANCIA}/comparacion_mhs_{run_id}"
    Path(save_dir).mkdir(parents=True, exist_ok=True)
