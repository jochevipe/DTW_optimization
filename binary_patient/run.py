"""
Análisis individual de una MH + Binary-Patient (estrategia A10).
Ejecuta 1 corrida, muestra métricas DTW por iteración, genera plot para paper.

Uso (desde la raíz del proyecto):
    python -m binary_patient.run
"""
...
def print_iteration_table(res: dict, inst: dict) -> None:
    ...
    for i in range(len(hist_fit)):
        ...
        if dtw.get("ready"):
            # The raw D2 condition is shown together with its patient streak.
            fire_patient = dtw["D2_vs_const"] <= dtw["theta_c"]
            fire_s = "  T  " if fire_patient else "  F  "
            row = (
                ...
                f"{dtw['trigger_streak']:4d} | {dtw.get('streak', 0):6d}"
            )
        else:
            row = (
...
def plot_paper(res: dict, inst: dict, save_dir: str = "results/binary_patient"):
    """
    Plot de 2 paneles para paper científico.

    Panel 1: Fitness + zonas explore + fire markers + óptimo
    Panel 2: D2 vs theta_c + fire markers (señal de decisión A10)
    """
    hist_fit = res["historial_fitness"]
    hist_dtw = res["historial_dtw"]
    hist_mode = res["historial_modos"]
    ...
    ax1.set_title(
        f"{mh_name} + DTW Binary-Patient — Convergence (seed={res.get('semilla', '?')})"
    )
    ax1.legend(loc="lower right", framealpha=0.9)
    ax1.grid(True, alpha=0.2, linestyle=":")

    # ─── Panel 2: D2 vs theta_c (señal de decisión A10) ──────────────────
    ax2.plot(
    ...
    ax2.set_title(
        r"DTW Patient Signal — Fire after sustained $D_2 \leq \theta_c$"
    )
