...
STRATEGY_LABEL = "Vanilla-Exploración"


def print_epoch_results(nombre: str, resultados: list, inst: dict):
    optimo = inst["optimo"]
    print(f"\n{'=' * 70}")
    print(f"  {nombre} ({STRATEGY_LABEL})")
    print(f"{'=' * 70}")
...
def generate_plots(resultados_por_mh: dict, inst: dict, save_dir: str):
    ...
    ax.set_title(f"{STRATEGY_LABEL} — All MHs (seed={SEMILLA})")
    ...
    for ext in ("png", "pdf"):
        path = f"{save_dir}/{STRATEGY_KEY}_{inst_name}_{INDICE_INSTANCIA}.{ext}"
        fig.savefig(path)
        print(f"  Saved: {path}")
...
def main():
    ...
    print("=" * 70)
    print(f"  RESULTADOS MAESTRO — {STRATEGY_LABEL}")
    print("=" * 70)
    ...
    inst_name = Path(RUTA_INSTANCIA).stem
    save_dir = f"results/{STRATEGY_KEY}/todos/{inst_name}_{INDICE_INSTANCIA}/comparacion_mhs_{run_id}"
    Path(save_dir).mkdir(parents=True, exist_ok=True)

    resultados_por_mh = {}

    for nombre, mh_class in MHS.items():
        print(f"\n  Ejecutando {nombre} ({STRATEGY_LABEL})...")
