"""
Script maestro de resultados — B3 D2-Direct.
    ...
    for res in resultados:
        ...
        if ready:
            ...
            print(
                ...
                f"D1={np.mean(d1s):7.1f} (+-{np.std(d1s):.1f}) | "
...
def generate_plots(resultados_por_mh: dict, inst: dict, save_dir: str):
    ...
    2 paneles:
      ...
      2. Intensidad (D2-direct) de cada MH
    """
    ...
    ax1.set_title(f"B3 D2-Direct — All MHs (seed={SEMILLA})")
    ...
    ax2.set_title(r"DTW Stagnation Signal — Intensity (B3 D2-direct, scale={})".format(SCALE))
    ...
    for ext in ("png", "pdf"):
        path = f"{save_dir}/b3_d2_{inst_name}_{INDICE_INSTANCIA}.{ext}"
        fig.savefig(path)
        print(f"  Saved: {path}")
...
def main():
    ...
    print("=" * 70)
    print("  RESULTADOS MAESTRO — B3 D2-Direct")
    print("=" * 70)
    ...
    inst_name = Path(RUTA_INSTANCIA).stem
    save_dir = f"results/b3_d2/todos/{inst_name}_{INDICE_INSTANCIA}/comparacion_mhs_{run_id}"
    Path(save_dir).mkdir(parents=True, exist_ok=True)
