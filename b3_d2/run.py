"""
Análisis individual de una MH + DTW B3 D2-Direct.
...
def print_iteration_table(res: dict, inst: dict) -> None:
    ...
    print(f"\n{'=' * len(hdr)}")
    print(
        f"  DTW Log (B3 D2-Direct) — {MH_CLASS.__name__} | "
    ...
    for i in range(len(hist_fit)):
        ...
        if dtw.get("ready"):
            d2 = dtw["D2_vs_const"]
            th_c = dtw["theta_c"]
            ratio = d2 / (th_c * SCALE + eps)
            row = (
                f"{i:4d} | {fitness:10.1f} | {mode:>7} | {intensity:4.2f} | "
                f"{d2:7.1f} | {th_c:7.1f} | {ratio:7.3f} | "
                f"{dtw['D1_vs_ramp']:7.1f} | {dtw['delta']:+8.1f}"
            )
        else:
            row = (
...
def plot_paper(res: dict, inst: dict, save_dir: str = "results/b3_d2"):
    """
    Plot de 2 paneles:
    Panel 1: Fitness + zonas explore sombreadas
    Panel 2: D2 + theta_c + curva de intensidad (eje secundario)
    """
    hist_fit = res["historial_fitness"]
    hist_dtw = res["historial_dtw"]
    hist_mode = res["historial_modos"]
    ...
    # --- Panel 2: D2 + theta_c + Intensity ---
    ax2.plot(
        ready_iters, d2_vals, color=COLORS["d2"],
        linewidth=1.3, label="D2 (dist. to constant)", zorder=3,
    )
    ax2.plot(
        ready_iters, theta_c_vals, color=COLORS["theta_c"],
        linewidth=1, linestyle="--", alpha=0.7,
        label=r"$\theta_c$", zorder=2,
    )
    # Shade region where D2 < theta_c (estancado)
    ax2.fill_between(
        ready_iters, d2_vals, theta_c_vals,
        where=[d < th for d, th in zip(d2_vals, theta_c_vals)],
        color=COLORS["d2"], alpha=0.06, label="D2 < θ_c (stagnant)",
    )
    ...
    ax2.set_ylabel("D2 / θ_c")
    ax2.set_title(r"DTW Stagnation Signal — D2 + $\theta_c$ + Intensity (B3 D2-direct)")
