"""
DTW-PhaseLock — base exploit con explore solo ante estancamiento profundo
confirmado (target: GeneticAlgorithm; también DE y GWO como control).
Corre todas las MHs, muestra métricas DTW en consola, y genera gráficos.

Uso (desde la raíz del proyecto):
    python -m dtw_phaselock.resultados
"""
    ...
    for res in resultados:
        ...
        if ready:
            ...
            print(
                ...
                f"Plateau min={PLATEAU_MIN} iters"
            )
...
def _get_explore_entries(hist_mode):
    """Iteraciones donde arranca un episodio de explore (exploit -> explore)."""
    entries = []
...
def generate_plots(resultados_por_mh: dict, inst: dict, save_dir: str):
    ...
    Genera 2 paneles estilo paper:
      1. Fitness superpuesto con segmentos punteados en explore + entradas a explore.
      2. D2 vs theta_c con los umbrales de entrada/salida de PhaseLock.
    """
    ...
    for nombre in nombres:
        ...
        explore_entries = _get_explore_entries(hist_mode)
        ...
        ax1.plot(
            ...
            label=f"{nombre} (max={res['mejor_fitness']:.1f}, explores={len(explore_entries)})",
        )

        # Marcadores de entrada a explore
        if explore_entries:
            entry_fits = [hist_fit[i] for i in explore_entries]
            ax1.scatter(
                explore_entries, entry_fits, color=COLORS["explore_marker"],
    ...
    ax1.set_title("DTW-PhaseLock — All MHs")
    ax1.legend(loc="lower right", framealpha=0.9)
    ax1.grid(True, alpha=0.2, linestyle=":")

    # =========================================================================
    # Panel 2: D2 vs theta_c (señal del controlador PhaseLock)
    # =========================================================================
    for nombre in nombres:
        res = mejores[nombre]
        ...
        if ready_iters:
            ...
            # Umbral de entrada a explore (D2 <= theta_c)
            ax2.plot(
                ready_iters, theta_c_vals, color=color,
                linewidth=0.8, linestyle=":", alpha=0.5,
                label=rf"$\theta_c$ {nombre}", zorder=2,
            )
            # Umbral de recuperación (D2 > 2.5*theta_c)
            recovery_line = [2.5 * t for t in theta_c_vals]
            ax2.plot(
                ready_iters, recovery_line, color=color,
    ...
    ax2.set_title(
        rf"DTW-PhaseLock — enter explore at $D_2 \leq \theta_c$ with "
        rf"$\geq {PLATEAU_MIN}$ no-improve iters; recover at $D_2 > 2.5\theta_c$"
    )
    ...
    for ext in ("png", "pdf"):
        path = f"{save_dir}/dtw_phaselock_{inst_name}.{ext}"
        fig.savefig(path)
        print(f"  Saved: {path}")
...
def main():
    ...
    print("=" * 70)
    print("  RESULTADOS MAESTRO — DTW-PhaseLock")
    print("=" * 70)
    ...
    print(f"  Plateau min: {PLATEAU_MIN} iteraciones sin mejora")

    # Crear carpeta de salida con timestamp
    run_id = datetime.now().strftime("%Y%m%d_%H%M%S")
    inst_name = Path(RUTA_INSTANCIA).stem
    save_dir = f"results/dtw_phaselock/todos/{inst_name}_{INDICE_INSTANCIA}/comparacion_mhs_{run_id}"
    Path(save_dir).mkdir(parents=True, exist_ok=True)
